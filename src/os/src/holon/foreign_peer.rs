...
impl ForeignPeer {
    ...
    pub fn send_handshake(&mut self, _private_key: &[u8; 32]) -> Result<(), &'static str> {
        match self.state {
...
// implement debug without shared_secret
impl fmt::Debug for ForeignPeer {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "ForeignPeer {{ epoch: {}, hid: {}, address: {}, pubkey: {:?}, state: {:?} }}",
            self.epoch,
            self.hid,
            self.address.get_addr(),
            self.pubkey,
