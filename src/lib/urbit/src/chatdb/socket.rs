...
pub fn sync_route() -> impl Filter<Extract = (impl warp::Reply,), Error = warp::Rejection> + Clone {
    let signaling = warp::path("hol" / "sync")
        .and(warp::ws())
        ...
        .map(
            |ws: warp::ws::Ws,
             ...
             forwarded_for_ips: Vec<IpAddr>,
             query_params: HashMap<String, String>| {
                ws.on_upgrade(move |socket| handle_sync(socket))
            },
        );
    signaling
}

pub async fn handle_sync(ws: WebSocket) {
